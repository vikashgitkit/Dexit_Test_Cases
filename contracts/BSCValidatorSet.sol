...
contract BSCValidatorSet is IBSCValidatorSet, System {
    ...
    uint8 public constant MISDEMEANOR_THRESHOLD = 2;
    uint8 public constant FELONY_THRESHOLD = 4;
    ...
    uint256 public constant Max_Validators = 4; // Total Max Validator(5)
    uint64 public constant StakingLockPeriod = 10; // Stake Locking Period(7 days) //201600 blocks
    uint64 public constant unjailingPeriod = 2 minutes; //2 days
    ...
    function pushContractOwner(address contAddr, address owner) external onlyCoinbase returns(bool) {
        IRewardRegister(CROSS_CHAIN_CONTRACT_ADDR).pushContractRecord(contAddr, owner);
        return true;
    }
